...
OTU_data_firmicutes_transpose<-(OTU_data)




library("pdist")
d <- pdist(T2D_data_firmicutes_transpose,OTU_data_firmicutes_transpose)
...
for (i in 1:68)
{
dist_OTUs[i]<-(dist(rbind(OTU_data_firmicutes[i,],OTU_data_firmicutes[59,])))
}
...
corr_mat=cor(t(OTU_data_firmicutes),method="spearman",use = "complete.obs")
